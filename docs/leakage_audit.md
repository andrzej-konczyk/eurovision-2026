...
| KL-LA-04 | `SimpleImputer(strategy="median", keep_empty_features=True)` in `NNPipeline` (`src/models/nn.py`). For CV folds that cover only 2016–2017, `implied_prob_close` is entirely NaN (no betting odds coverage for those years). Without `keep_empty_features=True`, scikit-learn silently drops the all-NaN column, causing a feature-count mismatch between folds and at inference time. | Very low — a column imputed to zero carries no outcome signal; the downstream `nan_to_num(…, nan=0.0)` in `fit()` and `predict_proba()` is applied identically, so the zero fill is symmetric. | **Conscious architectural decision.** Retaining the column ensures a consistent 23-feature input tensor across all folds and at inference time (2026 data, where odds are available). Removing the flag would silently break feature alignment. Not an oversight. |
